...
void keysetup(struct poet_ctx_t *ctx, const unsigned char key[KEYLEN])
{
    ...
    aes_expand_key(k_axu, ctx->aes_lt);

    k_axu = aes_encrypt(three, expanded_key);
    aes_expand_key(k_axu, ctx->aes_lb);
                                    ...
                                    __m128i* k, __m128i* lt, __m128i* lb)
{
    ...
    *x = _mm_aesenclast_si128(*x, lt[ROUNDS]);
    *y = _mm_aesenclast_si128(*y, lb[ROUNDS]);
